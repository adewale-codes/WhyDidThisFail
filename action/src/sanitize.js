...
// Secret redaction -- a deliberate, faithful port of web/lib/sanitize.ts's
// rule set to plain JS (the website is TypeScript/Next.js; this action runs
// as a standalone Node script with no build step, so it can't just
// `import` that module). Keep these two rule sets in sync.
//
...
// the Phase 1 API -- and neither the CLI nor the Phase 1 API sanitize
// anything themselves (Phase 1's job is diagnosis, not data hygiene; the
// CLI is a thin client). A real CI log can contain real secrets, so this
// module is applied to every fetched log BEFORE it touches the CLI, the
// diagnosis API, or (if configured) the website submission. See
// action/README.md's "Sanitization" section, and action/test/simulate.js
// for the test that proves this actually happens, not just assumes it.
