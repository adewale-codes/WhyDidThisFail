...
/**
 * Shells out to the Phase 2 CLI (`whyfail`) to get a diagnosis -- this is
 * the *only* place diagnosis logic is invoked from. The action never talks
 * to the Phase 1 API or an LLM directly, so there is nothing here to keep
 * in sync with that pipeline as it evolves.
 *
 ...
 * @returns {Promise<import('../../web/lib/types').DiagnoseResponse>}
 */
...
/**
 * Submits the (already sanitized) log to the Phase 3 website's own
 ...
 * known tradeoff in action/README.md rather than silently accepted.
 *
